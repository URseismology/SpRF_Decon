function [time, radRF, transRF, varR, varT] = ...
    Sp_MTCRF_se(R, T, Z, dnoi, PSI, K, Dt, fc)
% [time, radRF, transRF, varR, varT] = Sp_MTCRF_se(R, T, Z, dnoi, PSI, K, Dt, fc)
%
isplot = 0; isFreq = 1;
N    = length(Z);
...
So  = zeros(N, 1); % Changed to real zeros for the power spectrum
...
for k = 1: K
    ...
    YR = fDat(:,2);
    YT = fDat(:,3);
    Ynoi = fDat(:,4);  % <--- FIX 1: Store the noise FFT here instead of assigning to So
    ...
    % <--- FIX 2: Calculate the TRUE REAL power spectrum of the noise
    So = So + ( conj(Ynoi) .* Ynoi );

end

% <--- FIX 3: Average the noise power over the number of tapers
So = So / K;

% frequency domain RFs (Now dividing by a purely real denominator!)
HR = SZR ./ (SZZ + So);
HT = SZT ./ (SZZ + So);
